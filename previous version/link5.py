...
#background
        self.backx,self.backy=2,0#which part of the background is blit
    ...
    def run(self):
        ...
        if link.rect.collidepoint(450,676) and (self.backx,self.backy)==(-1,0):
            ...
            self.visback=self.floors[2]
            blitback()
        if link.rect.collidepoint(400,100) and (self.backx,self.backy)==(2,0):
            self.backx,self.backy=-2,0
...
class enemy(gameobj):
    ...
    def pathfind(self):#self is enemy
        ...
        def astar(self):
            ...
            end=(link.x//20,link.y//20)
   
     
            
            frontier=PriorityQueue()#creates a queue which has priorities and stores parents
            frontier.put(start,0)#adds start
                        ...
                        if 0<=nex_t[0]<=39 and 0<=nex_t[1]<=36:#if the path is not out of bounds
                            if floor[nex_t[0]][nex_t[1]]==0 :#slice 2d list 
                                ...
                                if nex_t not in cost_so_far or new_cost<cost_so_far[nex_t]:#to make sure we are not on the same square again unless the cost is less
                                    ...
                                    frontier.put(nex_t,priority)#puts nex_t in queue
    ...
    def itemdrop(self):
         itemdrop=choice(["rrupee","grupee","heart","arrow","bomb"])
    ...
    def phase3(self):
        if state!="attacking":
            if state=="normal":
                self.pathfind()
                self.movement="walk"+self.direction
            if (self.x//20,self.y//20)==(link.x//20,link.y//20):
...
link.x+=120
def loadenemies(file="",speed=0,health=0,ani=False,clas=enemy):
    ...
    if not ani:
        for i in epos:
            ...
            if ani==squidanimations:
                stat=(4,3)
            enem=clas(ani,stat[0],stat[1],(pos[2],pos[3]),pos[0],pos[1],"left")
    else:
        for i in epos:
            pos=(round(float(i[0])),round(float(i[1])),round(float(i[2])),round(float(i[3])))
