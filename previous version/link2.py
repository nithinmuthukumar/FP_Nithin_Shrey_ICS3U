...
#background
        self.backx,self.backy=2,3#which part of the background is blit
    ...
    #sentry
     ...
     #                   bullet.checkcollide(mainsprite)# if statement already does
        ...
        arrow.operate()
        boomerang.operate()
        #if bomb.location!=(g.backx,g.backy):
 #           g.allsprites.remove(bomb)
    ...
    #link collide checking and hurttime working
        ...
        for i in self.itemsprites:
            if i.location!=(self.backx,self.backy):
                self.itemsprites.remove(i)
...
heartups=[imageload(("heart.png"),0.04)]
bomb_up=[imageload(("bomb.png"),0.2)]
arrow_up=[imageload(("arrow.jpg"),0.5)]
