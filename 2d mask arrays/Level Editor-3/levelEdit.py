...
back = imageload("0.png",4,False)
print(back.get_rect())

level = loadMap("(-2,0).txt")
print(len(level))
print(len(level[0]))
running = True
myClock = time.Clock()
while running:
    ...
    if keys[K_l]:
        ...
        for i in range(4):
            try:
                level[gx+i][gy] = current
                level[gx][gy+i]=current
                level[gx+i][gy+i]=current
            except:
                pass
